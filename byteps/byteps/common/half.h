...
// This flag dictates rounding for the float2half() routine only (used generally
// on Windows), not the f16c lib or cuda v7.5 (or later) behavior which is fixed
// at round-to-nearest-even.
...
__host__ __device__ float __half2float_warp(
    const volatile __half& h) { /* NOLINT(*) */
  __half val;
...
class MSHADOW_ALIGNED(2) half_t {
 public:
...
#if MSHADOW_CUDA_HALF
  MSHADOW_XINLINE explicit half_t(const __half& value) { cuhalf_ = value; }
#endif  // MSHADOW_CUDA_HALF
  ...
  MSHADOW_XINLINE half_t operator+() { return *this; }
 ...
 private:
  ...
  static int32_t const fp32FractionMask =
      ~(~0u << fp32FractionBits);                                // == 0x7fffff
  static int32_t const fp32HiddenBit = 1 << fp32FractionBits;    // == 0x800000
  static int const shift = fp32FractionBits - fp16FractionBits;  // == 13
  static int const shiftSign = 16;
  static int32_t const expAdjust =
      127 - 15;  // exp32-127 = exp16-15, so exp16 = exp32 - (127-15)

  static int32_t const infN = 0x7F800000;  // flt32 infinity
  static int32_t const maxN =
      0x477FFFFF;  // max flt32 that's a flt16 normal after >> by shift
  static int32_t const minN = 0x38800000;  // min flt16 normal as a flt32
  static int32_t const maxZ =
      0x33000000;  // max fp32 number that's still rounded to zero in fp16
  ...
  MSHADOW_XINLINE uint16_t float2half(const float& value) const {
    ...
    if (v.si <= maxZ) {
      ...
      // If exp16 == 0 (just into the denorm range), then significant should be
      // shifted right 1. Smaller (so negative) exp16 values should result in
      // greater right shifts.
      uint32_t vshift = 1 - exp16;
      uint32_t significand = fp32HiddenBit | (v.ui & fp32FractionMask);
      v.ui = significand >> vshift;
      // The only time it's *not* OK to add 0x1000 (i.e. half the flt16 fraction
      // lsb) is when the lsb of the flt16 fraction == 0 (so not rounding up to
      // even) and the additional bits to the right of the lsb are 1000...
      // (including flt32 significand bits that may be lost during the above
      // vshift).  The first term below will always be true for vshift >=12
      // (since even the 'hidden bit' has been shifted to the right of the '1'
      // bit in 0x1000). And when vshift <= 11, both terms combine to make the
      // proper test of the flt32 significand bits, including those lost during
      // the vshift.
  ...
  MSHADOW_XINLINE uint16_t float2half(const volatile float& value) const
      volatile {  // NOLINT (*)
    Bits v;
    v.f = value;
    uint32_t sign = v.si & signN;  // grab sign bit
  ...
  MSHADOW_XINLINE float half2float(const volatile uint16_t& value) const
      volatile {  // NOLINT(*)
    Bits v;
    v.ui = value;
    int32_t sign = v.si & signC;
